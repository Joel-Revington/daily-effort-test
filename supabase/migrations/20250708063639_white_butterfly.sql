...
CREATE POLICY "Users can read task comments" ON task_comments FOR SELECT TO authenticated USING (
  EXISTS (
    ...
    AND (tasks.assignee_id = auth.uid() OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin')))
  )
);
...
CREATE POLICY "Users can manage own skill requests" ON skill_requests FOR ALL TO authenticated USING (user_id = auth.uid());
CREATE POLICY "Admins can manage all skill requests" ON skill_requests FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);
...
CREATE POLICY "Users can manage own feedback requests" ON feedback_requests FOR ALL TO authenticated USING (requested_by_id = auth.uid());
CREATE POLICY "Admins can read all feedback requests" ON feedback_requests FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
);
