...
CREATE POLICY "Users can manage own activities" ON report_activities
  FOR ALL TO authenticated
  USING (
    EXISTS (
...
CREATE POLICY "Admins can read all activities" ON report_activities
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can read task comments" ON task_comments
  ...
  USING (
    EXISTS (
      ...
      AND (assignee_id = auth.uid() OR assigned_by_id = auth.uid())
    ) OR
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() 
...
CREATE POLICY "All users can read software" ON software
  FOR SELECT TO authenticated
  USING (true);
...
CREATE POLICY "Users can manage own requests" ON skill_requests
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Admins can manage all requests" ON skill_requests
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can manage feedback for assigned tasks" ON feedback_requests
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM tasks 
      WHERE id = feedback_requests.task_id 
