...
CREATE TABLE IF NOT EXISTS feedback_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  task_id UUID REFERENCES tasks(id) ON DELETE CASCADE,
  requested_by_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
...
-- Software policies (read-only for all authenticated users)
CREATE POLICY "Authenticated users can read software" ON software
  FOR SELECT TO authenticated
  USING (true);
...
CREATE POLICY "Admins can read all reports" ON daily_reports
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can manage own report activities" ON report_activities
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM daily_reports 
      WHERE id = report_activities.report_id 
      AND user_id = auth.uid()
    )
  );

CREATE POLICY "Admins can read all report activities" ON report_activities
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can read comments on assigned tasks" ON task_comments
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM tasks 
...
CREATE POLICY "Admins can read all task comments" ON task_comments
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can add comments to assigned tasks" ON task_comments
...
CREATE POLICY "Admins can read all skills" ON user_skills
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can manage own skill requests" ON skill_requests
  FOR ALL TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Admins can manage all skill requests" ON skill_requests
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Admins can read all KPI entries" ON kpi_entries
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can read assigned leads" ON sales_leads
  FOR SELECT TO authenticated
  USING (assigned_to_id = auth.uid());

CREATE POLICY "Admins can manage all leads" ON sales_leads
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
...
CREATE POLICY "Users can update assigned leads" ON sales_leads
  FOR UPDATE TO authenticated
  USING (assigned_to_id = auth.uid());

-- Feedback requests policies
CREATE POLICY "Users can manage own feedback requests" ON feedback_requests
  FOR ALL TO authenticated
  USING (requested_by_id = auth.uid());

CREATE POLICY "Admins can read all feedback requests" ON feedback_requests
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
